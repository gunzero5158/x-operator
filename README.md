...
- **搜索规则**：关键词（**逗号隔开 = 命中任意一个**，`adult, nsfw, AI美女` 会自动变成 `adult OR nsfw OR "AI美女"`；
  也可直接写 X 语法）+ 语义条件 + 达标分 + **语言多选**（自动拼成 `(lang:ja OR lang:en)`），转推默认排除；
  每条规则显示实际查询、累计结果分布和「查看结果」直达按钮；运行后弹出结果框带跳转；试运行只打分不入库。
