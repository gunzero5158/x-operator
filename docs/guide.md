...
- **审核队列**：逐条看生成说明、改文案、**换素材**、批准/跳过/拉黑；每条标明来源（AI 匹配素材 / 手动选素材 / AI 撰写 / 定时）；
  ...
  **已跳过**的条目会用中文写明原因（作者冷却期内 / 该推文已回复过 / 作者在黑名单 / 条目已过时效 / 手动跳过），每条有**「重新判断」**，
  筛选到「已跳过」时顶部还有「重新判断全部已跳过」：按现在的情况再查黑名单、是否已回复过、作者冷却，都不成立就放回待审核
  （时效从现在重新计时，原推文可能已经比较旧，批准前自己掂量）；仍不通过的保持跳过并把原因更新成现在的。
...
```
x_operator/
  db/         schema.py(DDL v14) database.py(连接/自动迁移) seed.py(默认设置 + 旧演示数据清理)
