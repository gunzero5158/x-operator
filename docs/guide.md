...
看颜色就知道标签在说哪类信息，抓取记录 / 审核队列 / 规则 / 推主 / 定时发帖 / 素材库都一样，页顶有图例，鼠标悬停有解释：
...
| 靛蓝 / 橙 | **类型**：回复 / 发帖 | 回复、发帖 |
| 绿 / 蓝 / 橙 / 红 / 浅灰 | **状态**：正常 / 等待 / 待处理 / 失败 / 停用过期 | 已进审核队列、待匹配、达标但未生成回复、已停用 |
...
- **设置**：账号、自动运行、LLM 网关、合规参数、预算、黑名单（@handle 或数字 id 都能拦）、数据清理、素材附件占用空间统计（列出已无引用可放心删的文件，「前往清理」按钮直接打开 data/media/ 目录）。数字参数保存时会校验范围。
  ...
  - **界面显示时区**（自动运行页顶部，默认东京）：审核队列 / 抓取记录 / 定时发帖等所有页面上的创建、发送、下次运行时间都按它显示，
