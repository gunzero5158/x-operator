...
请从中选出最接近的一条。你选的只是草稿，之后一定会有人工审核，所以原则是「默认选一条」：
1. 这条推文已经通过相关性筛选，候选素材也是运营者专门为这类推文准备的——总是选出相对最贴切的那条，
   哪怕不完美。只有在所有素材都和推文完全不搭、发出去会显得莫名其妙时才跳过。
2. {polish_rule}
3. confidence 表示贴切程度：0.8~1 很贴切；0.5~0.7 沾边、可以用；0.3~0.4 勉强；只有跳过时才低于 0.3。
...
def length_line(limit: int) -> str:
    """拼在写作类提示末尾的长度要求。会员账号不设硬上限，但仍要求简短。"""
    if limit >= 1000:
        return "\n\n长度：这个账号订阅了 X Premium，没有硬上限，但请保持简短（一般不超过 280 单位）。"
    return f"\n\n长度：必须不超过 {limit} 个单位（中日韩每字 2 单位、链接 23 单位），建议留 10% 余量。"


# ---------------- 超长缩写 ----------------
SHORTEN_SYSTEM = """你是 X（推特）文案编辑。给定一条超过长度上限的推文，把它缩短到上限以内。
X 的长度按「计数单位」算：英文字母/数字/标点每个 1 单位，中文/日文/韩文/emoji 每个 2 单位，每个链接固定 23 单位。
铁律：
