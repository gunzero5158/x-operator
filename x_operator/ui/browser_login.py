...
def login_accounts(account_ids: list[int], refresh=lambda: None) -> None:
    ...
    try:
        ...
        return
    ui.notify("已加入登录队列；可以继续使用其他功能", type="positive")
    login_tasks(refresh)
...
def browser_setup(account_ids=None, refresh=lambda: None) -> None:
    with ui.dialog() as dlg, ui.card().classes("w-[560px] max-w-[95vw] gap-4"):
        ui.label("专用浏览器 · 可选组件").classes("text-lg font-bold")
        ui.label("密码登录需要下载专用 Chromium。Cookie 登录不需要安装，下载完成后可以重复使用。")
        hint("浏览器窗口会打开在运行服务的电脑上，需要本机有桌面环境。它不读取你日常 Chrome 的个人资料。下载使用系统代理；账号登录使用账号配置的代理。")
        ...
        with ui.row().classes("w-full justify-end gap-2"):
            ui.button("关闭，后台继续", on_click=dlg.close).props("flat")
            download_button = ui.button("下载 Chromium", icon="download", on_click=download).props("outline")
            start_button = ui.button("开始登录", icon="login", on_click=start)
        ...
        def tick():
            ...
            status.set_text("专用 Chromium 已就绪" if ready and not active else state["message"])
            download_button.set_text("重新下载" if ready else "重试下载" if state["status"] == "failed" else "下载 Chromium")
...
def login_tasks(refresh=lambda: None) -> None:
    with ui.dialog() as dlg, ui.card().classes("w-[780px] max-w-[95vw] max-h-[85vh] overflow-auto gap-4"):
        with ui.row().classes("w-full justify-between items-center"):
            ui.label("浏览器登录任务").classes("text-lg font-bold")
            ui.button("关闭，后台继续", on_click=dlg.close).props("flat")
        hint("一次处理一个账号。遇到额外验证，请在专用浏览器完成；也可以跳过当前账号，让后面的继续。任务不会因关闭此面板或切换页面而中断。服务重启后需重新发起。")
        ...
        def tick():
            ...
            with body:
                if not jobs:
                    ui.label("暂无登录任务").classes("text-sm")
                for job in jobs:
                    items = job["items"]
                    ...
                    with ui.card().classes("w-full gap-3"):
                        with ui.row().classes("w-full items-center justify-between"):
                            ui.label(f"已处理 {done}/{len(items)} · 成功 {success}").classes("font-semibold")
                            if not job["finished"]:
                                ui.button("取消本批", on_click=lambda jid=job["id"]: service.cancel(jid)).props("outline color=negative")
                            else:
                                retry = [i["account_id"] for i in items if i["status"] != "success"]
                                if retry:
                                    ui.button("重试未完成账号", icon="refresh",
                        ...
                        for item in items:
                            with ui.row().classes("w-full items-center gap-2"):
                                ...
                                if item["status"] in {"running", "manual"}:
                                    ui.button("跳过此账号", on_click=lambda jid=job["id"], aid=item["account_id"]: service.cancel(jid, aid)).props("flat dense")
...
def batch_import(refresh=lambda: None) -> None:
    ...
    with ui.dialog() as dlg, ui.card().classes("w-[760px] max-w-[95vw] max-h-[90vh] overflow-auto gap-3"):
        ui.label("批量导入账号").classes("text-lg font-bold")
        ui.label("导入账号密码后，使用与单账号相同的浏览器登录流程。每批最多 200 个账号。")
        with ui.expansion("格式与示例", icon="help_outline").classes("w-full"):
            ui.label("每行：用户名,密码,TOTP密钥,邮箱,代理。前两列必填；未启用两步验证可留空第三列。支持 CSV、制表符或 ---- 分隔。")
            ...
            ui.label("含逗号的密码请按 CSV 格式加双引号。使用账号 handle，不要把邮箱填在第一列。同名账号（含已删除账号）会跳过，不会覆盖已有设置。")
        content = ui.textarea("粘贴账号列表").props('outlined autogrow autocomplete="off" spellcheck="false"').classes("w-full")
        ...
        def inspect():
            ...
            with preview:
                for record in records:
                    ui.label(f"@{record['handle']} · " + ("已存在，将跳过" if record["handle"].lower() in existing else "将添加，等待登录"))
        ...
        def save():
            try:
                ...
                if parsed != records:
                    inspect()
                    error.set_text("内容已变化，请确认更新后的预览，再点击导入")
                    return
                ...
                error.set_text("导入未完成，数据库可能正忙，请稍后重试")
                return
            dlg.close()
            refresh()
            ui.notify(f"已添加 {len(ids)} 个账号，跳过 {skipped} 个重复账号", type="positive")
            if ids:
                login_accounts(ids, refresh)

        with ui.row().classes("w-full justify-end gap-2"):
            ui.button("取消", on_click=dlg.close).props("flat")
            ui.button("检查并预览", icon="fact_check", on_click=inspect).props("outline")
            submit = ui.button("导入并登录", icon="login", on_click=save)
            submit.set_enabled(False)
...
def toolbar(refresh) -> None:
    ...
    with ui.row().classes("w-full items-center gap-2"):
        ui.button("批量导入", icon="playlist_add", on_click=lambda: batch_import(refresh)).props("outline")
        ui.button("批量登录待处理账号", icon="login", on_click=lambda: pending_dialog(refresh)).props("outline")
        ui.button("登录任务", icon="pending_actions", on_click=lambda: login_tasks(refresh)).props("outline")
        ui.button("浏览器组件", icon="web", on_click=lambda: browser_setup(refresh=refresh)).props("flat")
        status = ui.label().classes("text-sm").props('role="status" aria-live="polite"')
    previous = None
    ...
    def tick():
        ...
        status.set_text((f"{len(active)} 个账号等待完成" + (" · 需要人工验证" if manual else "")) if active else "")
...
def pending_dialog(refresh) -> None:
    ...
    if not rows:
        ui.notify("没有等待登录的账号；已保存 Cookie 的账号可从卡片单独重新登录", type="info")
        return
    with ui.dialog() as dlg, ui.card().classes("w-[560px] max-w-[95vw] max-h-[85vh] overflow-auto gap-3"):
        ui.label("选择待登录账号").classes("text-lg font-bold")
        ui.label("列出已填密码但尚未取得 Cookie，或凭据失效的账号。每批最多 200 个。")
        selected = {}
        for row in rows:
        ...
        def start():
            ...
            if not 1 <= len(ids) <= 200:
                ui.notify("请选择 1–200 个账号", type="warning")
                return
            dlg.close()
            login_accounts(ids, refresh)
        ...
        with ui.row().classes("w-full justify-end gap-2"):
            ui.button("取消", on_click=dlg.close).props("flat")
            ui.button("开始登录", icon="login", on_click=start)
    dlg.open()
